...
# 平滑化幅（ウィンドウサイズ）を設定
window_size = 50  # 任意の数に変更可能

# 時間軸方向に移動平均を計算
amplitudes_ma = np.array([moving_average(amplitudes[:, i], window_size) for i in range(num_subcarriers)]).T

# 振幅をプロットするための初期化
fig, ax = plt.subplots()
plt.subplots_adjust(bottom=0.3)  # スライダーとの重なりを防ぐために調整
line, = ax.plot(range(num_subcarriers), amplitudes_ma[0], label="Amplitude Spectrum (Smoothed)")
ax.set_xlabel('Subcarrier Index')
ax.set_ylabel('Amplitude (Smoothed)')
ax.set_title('Subcarrier Amplitude Spectrum (Smoothed)', loc='left')  # タイトルを左に配置
ax.set_ylim(0, 1.1)  # y軸の上限を1.1に設定 (正規化された値の範囲に基づく)
...
slider = Slider(ax_slider, 'Time', 0, len(timestamps) - window_size, valinit=0, valstep=1)
...
def update_plot(index):
    ...
    current_index = index
    line.set_ydata(amplitudes_ma[current_index])
...
def on_key(event):
    global current_index
    if event.key == 'right' and current_index < len(timestamps) - window_size:
