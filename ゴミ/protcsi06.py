# 平滑化の間隔を引数で指定するように変更
# python script_name.py file1.csv file2.csv --smoothing_interval 1S
# python script_name.py file1.csv file2.csv --smoothing_interval 500ms
...
def plot_csi_data(file_path, ax, smoothing_interval):
    # CSVファイルを読み込む
    ...
    # ユーザー指定の間隔で平滑化（移動平均）
    smoothed_amplitudes_df = amplitudes_df.resample(smoothing_interval).mean()
...
def main(file_paths, smoothing_interval):
    ...
    for i, file_path in enumerate(file_paths):
        ...
        plot_csi_data(file_path, ax, smoothing_interval)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--smoothing_interval', type=str, default='1S', 
                        help='Smoothing interval (e.g., 1S for 1 second, 500ms for 500 milliseconds)')
    args = parser.parse_args()
    main(args.file_paths, args.smoothing_interval)
