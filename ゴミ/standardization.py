...
for row in data_column:
    ...
    # 振幅を標準化 (平均0、標準偏差1)
    standardized_amplitude = (amplitude - amplitude.mean()) / amplitude.std()
    
    amplitudes.append(standardized_amplitude)
    phases.append(phase)
...
line, = ax.plot(range(num_subcarriers), amplitudes[0], label="Amplitude Spectrum (Standardized)")
ax.set_xlabel('Subcarrier Index')
ax.set_ylabel('Standardized Amplitude')
ax.set_title('Subcarrier Amplitude Spectrum', loc='left')  # タイトルを左に配置
ax.set_ylim(-3, 3)  # y軸の範囲を設定 (標準化された値の範囲に基づく)
