...
def plot_data(file_path):
    ...
    # 移動平均フィルタ（ウィンドウ幅任意）を適用
    smoothed_amplitudes = amplitudes_df.rolling(window=50, min_periods=1).mean()

    # 平滑化後の連続する値の差分を計算して動きを検出
    long_term_diff = smoothed_amplitudes.diff(periods=10).abs()
    diff_amplitudes = long_term_diff
    # 動き検出の閾値を設定
    threshold = 6.0  # 感度に応じて調整
    ...
    def update_plot(index):
        ...
        # 各サブキャリアの変化が閾値を超えたかチェック
        if diff_amplitudes.iloc[current_index].max() > threshold:
